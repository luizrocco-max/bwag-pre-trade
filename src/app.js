...
(function () {
  ...
  function updateMeta() {
    ...
    box.innerHTML = `<div class="row gap-sm" style="justify-content:flex-end">
        ...
        <button class="btn btn-ghost btn-sm" id="topbar-del" title="Remover o fundo ativo (mantém os demais fundos, a política e o De-Para)">Remover fundo</button>
        <button class="btn btn-ghost btn-sm" id="topbar-reset" title="Recomeçar — remove todos os fundos e cenários (mantém política e De-Para)">Limpar</button>
      </div>
  ...
  function removerFundo(nome) {
    const i = STATE.fundos.findIndex(f => f.header.fundo === nome);
    if (i < 0) return;
    STATE.fundos.splice(i, 1);
    delete STATE.cenarios[nome]; delete STATE.cenarioAtivo[nome];
    if (STATE.fundoAtivo === nome) STATE.fundoAtivo = STATE.fundos.length ? STATE.fundos[0].header.fundo : null;
    persist(); renderStepper(); rerenderActive();
    toast('Fundo removido: ' + nome, 'ok');
  }
  function limparTudo() {
    ...
    if (!STATE.fundos.length) {
      ...
      html += STATE.fundos.map(fd => {
        ...
        return `<button class="btn ${on ? 'btn-primary' : 'btn-outline'} btn-sm" data-fundo="${esc(fd.header.fundo)}">
          ${esc(fd.header.fundo)} <span class="badge ${fd.formato === 'bradesco' ? 'badge-info' : 'badge-muted'}" style="margin-left:6px">${fd.formato === 'bradesco' ? 'Bradesco' : 'BTG'}</span>
          <span data-delfundo="${esc(fd.header.fundo)}" title="Remover este fundo" style="margin-left:8px;opacity:.55;cursor:pointer;font-weight:700">✕</span></button>`;
      }).join('');
      html += `<span class="small muted" style="align-self:center;margin-left:4px">— clique no fundo para ativar; no ✕ para remover</span>`;
      html += `</div>`;
    }
    html += `</div></div>`;
  ...
  function renderPolitica() {
    ...
    let html = `<div class="panel"><div class="panel-head"><div class="ph-text"><h2>Política de investimento & perfil</h2>
      ...
      <div class="ph-actions">
        <button class="btn btn-outline btn-sm" id="cfg-export" title="Baixar política + De-Para num arquivo para compartilhar com a equipe">Exportar config</button>
        <label class="btn btn-outline btn-sm" title="Carregar uma configuração compartilhada">Importar config<input type="file" id="file-cfg" accept=".json,application/json" hidden></label>
  ...
  function ruleReference() {
    ...
    let h = `<div class="panel"><div class="panel-head"><div class="ph-text"><h2>Catálogo regulatório de referência${help('Lista fixa embutida no app com as 69 regras de pré-trade da ANBIMA e da CVM 175 — é a mesma para todos os fundos e NÃO vem dos seus PDFs nem do Quantum. É a base normativa que o app usa: as checagens automáticas mais acima aplicam essas regras aos seus dados importados; aqui você vê o catálogo completo, inclusive as que precisam ser conferidas à mão.')}</h2><p>${R.length} regras de pré-trade mapeadas (ANBIMA · CVM 175). As checagens automáticas acima cobrem os itens quantificáveis a partir dos dados importados.</p></div></div><div class="panel-body tight"><div class="table-wrap"><table class="data"><thead><tr><th>Regra</th><th>Categoria${help('Tema da regra, para agrupar: concentração, liquidez, suitability, risco de mercado, crédito privado, vedações, etc.')}</th><th>Severidade${help('O que acontece se a regra for violada. Bloqueante = impedimento: a ordem não deve seguir sem correção (é uma quebra regulatória). Alerta = exige atenção/justificativa, mas não trava. Info = apenas informativo, sem limite a cumprir.')}</th><th>Limite padrão${help('Limite de referência da norma (ex.: "10% do PL da classe"). É o padrão regulatório; a política de investimento do próprio fundo pode ser mais restritiva e, nesse caso, prevalece.')}</th><th>Fonte${help('O dispositivo de onde a regra vem: artigo/anexo da Resolução CVM 175, da CVM 21 ou das Regras e Procedimentos da ANBIMA (AGRT). É a referência para auditoria.')}</th></tr></thead><tbody>`;
    R.forEach(r => {
  ...
  function exportarConfig() {
    const nP = Object.keys(STATE.politicas || {}).length, nD = Object.keys(STATE.depara || {}).length;
    const data = { tipo: 'bwag-pretrade-config', versao: 1, exportadoEm: new Date().toISOString().slice(0, 10),
      politicas: STATE.politicas, depara: STATE.depara };
    const blob = new Blob([JSON.stringify(data, null, 2)], { type: 'application/json' });
    const a = document.createElement('a'); a.href = URL.createObjectURL(blob);
    a.download = 'bwag-pretrade-config.json'; a.click();
    toast(`Configuração exportada (${nP} política(s), ${nD} De-Para)`, 'ok');
  }
  async function importarConfig(file) {
    if (!file) return;
    try {
      const d = JSON.parse(await file.text());
      if (d.tipo !== 'bwag-pretrade-config') { toast('Arquivo não é uma configuração do Pré-Trade', 'err'); return; }
      if (d.politicas) STATE.politicas = Object.assign({}, STATE.politicas, d.politicas);
      if (d.depara) STATE.depara = Object.assign({}, STATE.depara, d.depara);
      if (STATE.quantum) STATE.quantum.index.overrides = STATE.depara;
      persist(); rerenderActive();
      toast(`Configuração importada (${Object.keys(d.politicas || {}).length} política(s))`, 'ok');
    } catch (e) { toast('Erro ao ler configuração: ' + e.message, 'err'); }
  }
  ...
  document.addEventListener('click', (e) => {
    ...
    if (t.id === 'cfg-export') return exportarConfig();
    ...
    if (t.dataset.fundo) { STATE.fundoAtivo = t.dataset.fundo; persist(); renderStepper(); rerenderActive(); return; }
